...
use sol::animation::Clock;
use sol::layout::{ActivateWindow, AddWindowTarget, LayoutElement as _, Options, SizingMode};
use sol::render_helpers::{RenderCtx, RenderTarget};
use sol_config::{Color, OutputName, PresetSize};
...
pub struct Layout {
    ...
    clock: Clock,
    layout: sol::layout::Layout<TestWindow>,
    start_time: Duration,
    ...
    pub fn new(args: Args) -> Self {
        ...
        let options = Options {
            layout: sol_config::Layout {
                focus_ring: sol_config::FocusRing {
                    off: true,
                    ..Default::default()
                },
        ...
        let mut layout = sol::layout::Layout::with_options(clock.clone(), options);
