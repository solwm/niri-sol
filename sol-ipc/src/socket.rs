...
/// Name of the environment variable containing the sol IPC socket path.
pub const SOCKET_PATH_ENV: &str = "SOL_SOCKET";
    ...
    pub fn connect() -> io::Result<Self> {
        let socket_path = env::var_os(SOCKET_PATH_ENV).ok_or_else(|| {
            io::Error::new(
                ...
                format!("{SOCKET_PATH_ENV} is not set, are you running this within sol?"),
            )
        })?;
