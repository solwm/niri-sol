...
impl FocusRing {
    pub fn new(config: sol_config::FocusRing) -> Self {
        Self {
    ...
    pub fn update_config(&mut self, config: sol_config::FocusRing) {
        self.config = config;
    }
    ...
    pub fn config(&self) -> &sol_config::FocusRing {
        &self.config
    }
}
